...
func (data *entityData) findRow(tableNo int32, rowId int64) int {
	var idx int = sort.Search(data.children.Len(), func(i int) bool {
	...
	if idx >= 0 && idx < data.children.Len() {
		if data.children[idx].tableNo == tableNo && data.children[idx].pk == rowId {
			return idx
		}
	}
...
func (data *entityData) rowUpdated(tableNo int32, rowId int64) {
	if tableNo == 0 {
		...
		var idx int = data.findRow(tableNo, rowId)
		if idx >= 0 {
...
func (t *Table) storeRow(txn *sql.Tx, row reflect.Value, logger entityLog) error {
	var res sql.Result
	var stmt *sql.Stmt
	var e error
	var pk int64
...
func (c *ChildTable) storeChildRow(txn *sql.Tx, row reflect.Value, parentId int64, logger entityLog) error {
